...
def initialize(context):
    ...
    context.tb_log_dir = './log/pnews_reward_with_tao_and_window'
    context.tensorboard = TensorBoard(log_dir=context.tb_log_dir)
    context.target_profit_multiplier=3
    ...
    if not os.path.exists('history_data'):
        ...
        context.history_data = history_data
    if not os.path.exists('trading_title.pkl'):
        sys.exit(1)
    else:
        title_vec = pd.read_csv('trading_title')
        title_vec.index = title_vec.date
        title_vec = title_vec.drop('date', axis=1)
        context.doc2vec = title_vec
    ...
    context.model = DRL_Portfolio(feature_number=len(context.assets) * 8 + 100, asset_number=len(context.assets) + 1, object_function='reward')
...
def before_trading_start(context, data):
    ...
    # doc_vector = context.doc2vec[:str(trading_date)]
    doc_vector = context.doc2vec[:str(trading_date)]
    batch_f = batch_f.join(doc_vector)
...
if __name__ == '__main__':
    ...
    result.to_pickle('portfolio_news_backtest_result')
