...
class DRL_Single(object):
    def __init__(self, feature_number,dense_units_list=[1024, 768, 512, 256, 128], rnn_hidden_layer_number=4, rnn_hidden_units_number=128, learning_rate=0.001):
        ...
        self.output_rnn_init_state = tf.placeholder(tf.float32, [1, 1], name='rnn_output_initial_state')
        ...
        self.last_rnn_output = tf.placeholder(dtype=tf.float32, shape=[1, 1], name='previous_rnn_output')
        ...
        with tf.variable_scope('rnn', initializer=tf.contrib.layers.xavier_initializer(uniform=False), regularizer=tf.contrib.layers.l2_regularizer(0.01)):
            ...
            rnn_output_cell = self._add_gru_cell(1)
