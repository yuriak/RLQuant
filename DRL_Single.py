...
class DRL_Portfolio(object):
    def __init__(self, feature_number, asset_number, dense_units_list=[1024, 768, 512, 256, 128], rnn_hidden_layer_number=4, rnn_hidden_units_number=128, learning_rate=0.001):
        ...
        with tf.variable_scope('reward'):
            self.reward_t = tf.log(self.z)*self.action[:-1] - self.c * tf.abs(self.action[1:] - self.action[:-1])
            ...
            self.true_reward = tf.exp(self.cum_reward)
    ...
    def trade(self, feed):
        rewards, cum_reward, actions, current_state, current_rnn_output = self.session.run([self.reward_t, self.true_reward, self.action, self.current_state, self.current_output], feed_dict=feed)
